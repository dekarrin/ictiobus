...
func (g Grammar) DeriveFullTree(fakeValProducer ...map[string]func() string) ([]types.ParseTree, error) {
	...
	// both trees in a depth-first manner, the point at which the
	...
	// such nodes, which may even be one of the nodes that matched the
	...
	// TODO: re-examine splitting, we need to have a separate indication for
	for i := 0; i < len(finalTrees); i += 2 {
		left, right := finalTrees[i], finalTrees[i+1]
		path, diff := left.PathToDiff(right, true)
		if !diff {
			panic("trees are identical, should never happen")
		}

		lFollow := left.Follow(path)
		rFollow := right.Follow(path)
		isSC := strings.HasPrefix(lFollow.Value, types.ShortCircuitPrefix) || strings.HasPrefix(rFollow.Value, types.ShortCircuitPrefix)

		var podPath []int

		// if the divergence is due to the number of children, and not the value
		// or type of the node, then we can use the node itself as the point of
		// divergence.
		if len(lFollow.Children) == len(rFollow.Children) && lFollow.Terminal == rFollow.Terminal && (isSC || lFollow.Value == rFollow.Value) {
			podPath = path
		} else {
			// if the original path was to root, then it is not possible to
			// merge the trees.
			if len(path) == 0 {
				continue
			}

			// If the divergence is due to the value or type of the
			// node, then we need to use the parent node as the point of divergence.
			podPath = path[:len(path)-1]
		}

		// 2. In one of the trees, check the path from the point of divergence up
		// to the start symbol and check for any rule that can recurse and in the
		// process of doing so ends up producing the symbol of the pov node
		// at least once.
		var reachableRecursablePath []int
		var reachableRecursableDerivation []Pair[string, Production]
		for i := len(podPath) - 1; i >= 0; i-- {
			node := left.Follow(podPath[:i])
			// naive; might be recursable from multiple methods and we should
			// go through all of them, not just the first recursion.
			// TODO: implement above in future.

			reachable, derivationPath := g.ReachableFrom(node.Value, node.Value)
			if reachable {
				// but does the reachability find the sub-node somewhere in the
				// derivation?
				...
				reachableRecursableDerivation = derivationPath
				break
			}
		}
		...
		// 3. say we found [S] -> [E] -> [F] ...
		//                                ^ point of divergence in left
		//                         ^ reachable recursable node
		// (pod in right is [F'])
		//
		//
		// reachable means theres some path from [E] to E && F'.
		//
		// If we found:
		// [S] -> [E] -> [F']
		// [S] -> [E] -> [F]
		...
		// 	              ^ reachable recursable node
		//
		// reachable means theres some path from [F] to F && F'.
		//
		// we'd want an F =*=> g that can reach both F and F'.
		//
		// [S] -> [E1] -> [E2] -> [F]
		// [S] -> [E1] -> [E2] -> [F']
		//         ^ rr            ^ pod
		//
		// reachable means theres some path from E1 to E1 && F'
		//
		...
		//
		// store all children of [E] separately.
		// follow the path of derivation to set [E]'s children.
		// in the new subtree of [E], set all to shortest except for final E
		//
	}
