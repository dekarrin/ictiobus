...
rundir=".extestruns/$(date '+%Y%m%d%H%M%S')"
...
# assumes we dont put spaces in subdirs of "examples".
for f in $(cd examples ; echo */)
do
  test_failed=
  testdir="$rundir/$f"
  mkdir -p "$testdir"
  ...
  then
    ...
    if ! diff -u "examples/$f/test-expect.txt" "$testdir/actual.txt"
    then
