...
This example performs immediate evaluation. Its IR returned is a slice of
numbers, where the item with index N is the value of evaluating the Nth
statment. If you want to try it out, be sure and use the Go code defined below
it for the HooksTable.

    ```fishi
    %%actions

    # This actions section creates an IR that is the result of evaluating the
    # expressions. It will return a slice of FMNumber, one FMNumber for each
...
correct) for a fully-featured interpretation engine:
